...
@section('content')
    <div class="container">
        <div class="row justify-content-center align-items-center">
            ...
            <div class="col-md-11">
                <div class="row mb-1">
                    <h2>Editar de citas</h2>
                </div>

                <form method="POST" action="{{route('update.cita',$Cita->id)}}">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                        <div class="col-12">
                            ...
                            @if($errors->any())
                                <div id="" class="alert alert-danger" role="alert">
                                    Uno de los campos obligatorios esta vacío
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <input class="form-control {{$errors->has('fecha') ? 'is-invalid' : ''}}" type="date" id="fecha" name="fecha" value="{{ old('fecha',$Cita->fecha)}}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="time" class="form-control {{$errors->has('hora') ? 'is-invalid' : ''}}" id="hora" name="hora" value="{{ old('hora',$Cita->hora) }}" >
                                <div class="invalid-feedback">
                                    @if($errors->has('hora'))
                                        {{$errors->first('hora')}}
                                    @endif
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control {{$errors->has('motivo') ? 'is-invalid' : ''}}" id="motivo" name="motivo" rows="3">{{ old('motivo',$Cita->motivo) }}</textarea>
                                <div class="invalid-feedback">
                        ...
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select {{$errors->has('tipo_cita') ? 'is-invalid' : ''}}" aria-label="Default select example" id="tipo_cita" name="tipo_cita">
                                    ...
                                    <option value="Primera visita" {{ old('tipo_cita') == 'Primera visita' || $Cita->tipo_cita == 'Primera visita' ? 'selected' : '' }}>Primera visita</option>
                                    <option value="Cita derivida" {{ old('tipo_cita') == 'Cita derivida' || $Cita->tipo_cita == 'Cita derivida' ? 'selected' : '' }}>Cita derivida</option>
                                    <option value="Urgencia" {{ old('tipo_cita') == 'Urgencia' || $Cita->tipo_cita == 'Urgencia' ? 'selected' : '' }}>Urgencia</option>
                                    <option value="Empieza tratamiento" {{ old('tipo_cita') == 'Empieza tratamiento' || $Cita->tipo_cita == 'Empieza tratamiento' ? 'selected' : '' }}>Empieza tratamiento</option>
                                    <option value="Continua tratamiento" {{ old('tipo_cita') == 'Continua tratamiento' || $Cita->tipo_cita == 'Continua tratamiento' ? 'selected' : '' }}>Continua tratamiento</option>
                                    <option value="Revision" {{ old('tipo_cita') == 'Revision' || $Cita->tipo_cita == 'Revision' ? 'selected' : '' }}>Revision</option>
                                </select>
                                <div class="invalid-feedback">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input id="inputCedulaPersona" type="text" class="form-control {{$errors->has('persona_id') ? 'is-invalid' : ''}}" placeholder="" aria-label="Example text with button addon" aria-describedby="buttonModalPersona" value="{{old('persona_name',$Cita->persona->nombres.' '.$Cita->persona->apellidos)}}" disabled>
                                <div class="invalid-feedback">
                            ...
                            <input type="hidden" name="persona_id" id="persona_id" value="{{old('persona_id',$Cita->persona_id)}}">
                            <input type="hidden" name="persona_name" id="persona_name" value="{{old('persona_name',$Cita->persona->nombres.' '.$Cita->persona->apellidos)}}">
                            <input type="hidden" name="persona_cedula" id="persona_cedula" value="{{old('persona_cedula',$Cita->persona->cedula)}}">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="text" class="form-control" placeholder="" aria-label="Example text with button addon" aria-describedby="button-addon1" value="{{$Cita->estado}}" disabled>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <button class="btn btn-primary"><i class="bi bi-clipboard2-check"></i> Actualizar Cita</button>
...
<script>
    $(document).ready(function(){
        @if ($errors->any())
            ...
            @endif
        @else
            @foreach ($Cita->servicios_citas as $ser)
                seleccionarservicio({{$ser->id}},'{{$ser->nombre}}',{{$ser->precio}},{{$ser->descuento}},{{$ser->importe}},{{$ser->iva}},{{$ser->retencion}},{{$ser->subtotal}});
            @endforeach
        @endif
